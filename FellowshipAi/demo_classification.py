...
def classification_run_maml(folder, f_load, meta_model):
    # Compute error rate for one run of one-shot classification
    #
    # Input
    #  folder : contains images for a run of one-shot classification
    #  f_load : itemA = f_load('file.png') should read in the image file and process it
    #  meta_model : MetaCNN object responsible for one shot classification
    #
    # Output
    #  perror : percent errors (0 to 100% error)
    #
    ...
    task = [torch.stack(train_items + test_items).to(device)]

    y_softmax = meta_model.eval(task)

    YHAT = y_softmax.argmax(dim=-1)
...
def main(maml=None):
    ...
    if maml:
        print('One-shot classification demo with MAML')
        perror = np.zeros(nrun)
        for r in range(1, nrun + 1):
            rs = str(r)
            if len(rs) == 1:
                rs = '0' + rs
            perror[r - 1] = classification_run_maml('run' + rs, read_maml, maml)
            print(" run " + str(r) + " (error " + str(perror[r - 1]) + "%)")
        total = np.mean(perror)
        print(" average error " + str(total) + "%")
